...
- **Exit** = detected by FUSING two cues (per the domain owner): (a) a rapid **jerk** — `|a|` in the min-pooled envelope drops from a steady ~1 g to below ~0.35 g *quickly* (the rate of the drop, not just the low level; a ground bump also reads low but does not come from a sustained ~1 g), and (b) a **sharp background change** — before exit the aircraft fills most of the frame; at exit it rapidly shrinks and leaves as sky takes over. (A plain exposure/brightness jump, dark cabin → bright sky, also often marks exit but FAILS when the operator has already climbed out and is hanging outside the plane before exit; the background-change cue still works there.) This plan implements the telemetry jerk (came-from-~1g rapid drop); the visual cue is a fusion input to add once keyframe pixels are available (recon already extracts keyframes).
...
**Out of scope (next plan — visual detectors / fusion):** per the domain owner, the tandem **canopy opening** is detected strictly **visually** — a sharp growth of the object near the frame **centre** — optionally corroborated by a head-jerk; and the **exit** detection fuses this plan's telemetry jerk with the **sharp background change** at exit (the aircraft fills most of the frame, then rapidly leaves — more robust than a plain exposure jump, which fails when the operator is already hanging outside the plane before exit). Both need extracted-keyframe pixels, so they belong with the visual / feature-extraction work. This plan's accel opening-shock is only a sampling-window bound, never the emitted tandem canopy event.
