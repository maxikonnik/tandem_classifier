...
- **Scope: external operator camera only.** These cues (aircraft-leaving-frame exit, object-growth canopy) are tuned to the external camera. The instructor's hand-cam — close-up POV of the passenger, without the "aircraft leaves the frame" / "operator approaches the pair" dynamics — is a **separate plan**; its files are also mostly per-scene, but a single exit→landing file may additionally need within-file segmentation.
